...
//  var inputethn = d3.select("#Ethnicity").property('value');
    ...
    if (inputDate)
        filtered_data = filtered_data.filter(d => Date.parse(d.date) == Date.parse(inputDate));
    if (inputrace)
        filtered_data = filtered_data.filter(d => d.race.toLowerCase() == inputrace.toLowerCase());
     if (inputage)
         filtered_data = filtered_data.filter(d => d.age.toLowerCase() == inputage.toLowerCase());
    //  if (inputethn)
